...
class TestExpenseCreation:
    ...
    def test_create_expense_in_nonexistent_group_raises_error(self, expense_service, user_alice):

        with pytest.raises(ValueError, match="Group with id 'fake-group-id' not found"):
            expense_service.create_expense(group_id="fake-group-id", amount=50.0, payer=user_alice)
