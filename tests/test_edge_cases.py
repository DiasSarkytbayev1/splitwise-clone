...
class TestDineAndDash:
    ...
    def test_can_drop_out_after_settling_debt(
        ...
        expense_service.settle_up(group_id=group.id, payer=user_bob, payee=user_alice, amount=40.0)
...
class TestSoloSurvivor:
    def test_last_person_in_group_can_add_expense(self, group_service, expense_service, user_alice):

        group = group_service.create_group("Solo Trip", "USD", user_alice)

        expense = expense_service.create_expense(
            group_id=group.id, amount=10.0, payer=user_alice, debtors={user_alice}
        )
...
class TestDropOutFromExpense:
    ...
    def test_cannot_remove_last_debtor(self, group_service, expense_service, user_alice, user_bob):
        group = group_service.create_group("Test Group", "USD", user_alice)
        group_service.invite_to_group(group.id, user_bob)

        expense = expense_service.create_expense(
            group_id=group.id, amount=50.0, payer=user_alice, debtors={user_bob}
        )
    ...
    def test_drop_out_from_nonexistent_expense(self, expense_service, user_bob):
...
class TestLateJoiner:
    def test_late_joiner_not_in_old_expenses(
        ...
        new_expense = expense_service.create_expense(
            group_id=group.id, amount=50.0, payer=user_bob, debtors=None
        )
...
class TestMultipleExpensesComplexScenario:
    def test_complex_scenario_weekend_trip(
        ...
        expense_service.create_expense(
            group_id=group.id, amount=150.0, payer=user_alice, debtors={user_alice, user_bob}
        )
...
class TestZeroAmountEdgeCases:
    def test_very_small_amounts_handling(
        ...
        expense_service.create_expense(
            group_id=group.id, amount=0.01, payer=user_alice, debtors={user_alice, user_bob}
        )
