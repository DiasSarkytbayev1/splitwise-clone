...
class TestMinimumTransactions:
    def test_plan_never_exceeds_n_minus_one_transactions(
        ...
        expense_service.create_expense(
            group.id, 300.0, user_alice, {user_alice, user_bob, user_charlie}
        )
        expense_service.create_expense(
            group.id, 60.0, user_bob, {user_alice, user_bob, user_charlie}
        )
        expense_service.create_expense(
            group.id, 90.0, user_charlie, {user_alice, user_bob, user_charlie}
        )
    ...
    def test_plan_fully_settles_all_debts(
        ...
        expense_service.create_expense(
            group.id, 400.0, user_alice, {user_alice, user_bob, user_charlie, user_dave}
        )
        expense_service.create_expense(
            group.id, 80.0, user_bob, {user_alice, user_bob, user_charlie, user_dave}
        )
    ...
    def test_no_self_payments_in_plan(
        ...
        expense_service.create_expense(
            group.id, 90.0, user_alice, {user_alice, user_bob, user_charlie}
        )
    ...
    def test_all_amounts_are_positive(
        ...
        expense_service.create_expense(
            group.id, 60.0, user_alice, {user_alice, user_bob, user_charlie}
        )
...
class TestAlreadySettled:
    def test_empty_group_has_empty_plan(self, group_service, expense_service, user_alice, user_bob):
        group = group_service.create_group("Trip", "USD", user_alice)
        group_service.invite_to_group(group.id, user_bob)
